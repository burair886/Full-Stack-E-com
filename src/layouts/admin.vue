<template>
  <q-layout view="lHh Lpr lFf" class="tw-bg-white">
    <q-header class="bg-white tw-border-b-2 tw-border-[#f0f0f0]">
      ...
      <!-- sub navbar -->
      <q-toolbar
        class="md:tw-px-8 tw-px-4 tw-items-center tw-space-x-2 tw-border-t-2 tw-min-h-0"
      >
        <div class="tw-flex-shrink-0 tw-flex tw-items-center">
          <template v-for="shop in shops" :key="shop.id">
            <div class="tw-text-black shopnameintoolbar">
              <p
                class="tw-text-xl tw-transition-all tw-duration-700 tw-py-3"
                v-if="shop.id == route.params.shopId"
              >
                {{ shop.name }}
              </p>
            </div>
          </template>
          <span class="tw-ml-3 tw-p-1">
            <svg
    ...
    <q-page-container class="tw-min-h-screen tw-flex tw-flex-row">
      ...
      <div class="tw-w-full tw-p-6 tw-mb-8">
        <main class="tw-flex-1">
          <!-- <div
            class="row items-center justify-center tw-mb-5"
            v-if="!isEmailVerified"
          >
            <q-banner inline-actions class="text-white bg-primary full-width">
              ...
              <template v-slot:action>
                <q-btn
                  ...
                  color="white"
                  label="Resend"
                  @click="resendEmailVerifLink"
                  :loading="auth.emailLoading"
                />
              </template>
            </q-banner>
          </div> -->
